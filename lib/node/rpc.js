...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add('getexplicitmempool', this.getExplicitMempool);
  ...
  async getBlock(args, help) {
    ...
    return this.blockToJSON(entry, block, details, view);
  }
  ...
  async getExplicitMempool(args, help) {
    if (help || args.length > 1)
      throw new RPCError(errs.MISC_ERROR, 'getexplicitmempool ');
    ...
    for (var entry of this.mempool.map.values())
      out.push(this.txToJSON(entry.tx))
    return out;
  }
  ...
  async blockToJSON(entry, block, details, view) {
    const mtp = await this.chain.getMedianTime(entry);
    const next = await this.chain.getNextHash(entry.hash);
    ...
    for (let [index, tx] of block.txs.entries()) {
      entry.index=index
      if (details) {
        const json = this.txToJSON(tx, entry, view);
        txs.push(json);
        continue;
      }
      txs.push(tx.txid());
