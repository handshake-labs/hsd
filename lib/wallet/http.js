...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get('/wallet/:id/importnonce/:name', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const name = valid.str('name');
      const addr = valid.str('address');
      const bid = valid.ufixed('bid');
      ...
      const blind = await req.wallet.generateBlind(nameHash, address, bid);

      return res.json(200, {blind: blind.toString('hex')});
    });
    ...
    // TODO description
    this.post('/wallet/:id/registermany', async (req, res) => {
      const names = req.body.names
      const reso = req.body.resource
      const resource = Resource.fromJSON(reso);
      const tx = req.wallet.registerMany(names, resource);
    ...
    // Sends all possible reveals with 500 names per tx
    this.get('/wallet/:id/graduallyreveal', async (req, res) => {
      await req.wallet.graduallySendReveals();
      // if (sign)
        // await req.wallet.sign(tx, passphrase);
      // res.json(200, tx.getJSON(this.network));
    });

    // Sends all possible redeems with 500 names per tx
    this.get('/wallet/:id/graduallyredeem', async (req, res) => {
      await req.wallet.graduallySendRedeems();
    });

    // Sends all registers for given names with 500 names per tx
    this.post('/wallet/:id/graduallyregister', async (req, res) => {
      const names = req.body.names
      const reso = req.body.resource
      const resource = Resource.fromJSON(reso);
      await req.wallet.graduallyRegister(names, resource);
    });

    // Sends transfer of many names for a single address
    this.post('/wallet/:id/graduallytransfer', async (req, res) => {
      const names = req.body.names
      const address = req.body.address
      const addr = Address.fromString(address, this.network);
      const result = await req.wallet.graduallyTransfer(names, addr);
      res.json(200, result);
    });

    // Create Finalize
    this.post('/wallet/:id/graduallyfinalize', async (req, res) => {
      const names = req.body.names
      const result = await req.wallet.graduallyFinalize(names);
      res.json(200, result);
    });
    ...
    // Wallet Name States
    this.get('/wallet/:id/namebypage', async (req, res) => {
      const valid = Validator.fromRequest(req);
