...
class Wallet extends EventEmitter {
  ...
  async makeRevealAll() {
    ...
    // for (const {nameHash, prevout, own} of bids.slice(4700,5000)) {
    // var i, j, temparray, chunk = 500;
    // for (i=0,j=bids.length; i<j; i+=chunk) {
    // const start = 20000
    // console.log(bids.slice(start, start+5000))
    // for (i=start,j=bids.slice(start, start+5000).length; i<j; i+=chunk) {
      // temparray = bids.slice(i,i+chunk);
...
async myReveal() {
  ...
  bids = bids.filter(x => x.value != -1);
  console.log(bids.length)
  var a1  = Buffer.from("xn--2lwo25g", "ascii");
  var a2  = Buffer.from("xn--gswm14b", "ascii");
  var a3  = Buffer.from("xn--3jym0s", "ascii");
  // var a  = Buffer.from("xn--2lwo25g", "ascii");
  var a4  = Buffer.from("xn--netp8sxnv", "ascii");
  var a5  = Buffer.from("xn--ces85b", "ascii");
  var a6  = Buffer.from("xn--79qr84b", "ascii");
  var a7  = Buffer.from("xn--qruw06a", "ascii");

  bids = bids.filter(x => x.name != a1 && x.name != a2 && x.name != a3 && x.name != a4 && x.name != a5 && x.name != a6 && x.name != a7);
  console.log(bids.length)

  var i, j, temparray, chunk = 900;
  // var voin = await this.getCoin(Buffer.from("05e205426612a8611796f046ecf8007a408b961696e42454551cccbca3aa62a9"), 24);
  // coins = coins.filter(x => x.address == "hs1q8juy58gjrgad597wn6ej9dptqh7uq9j8epguq5")
  // console.log(voin)
  var coins = await this.getCoins();
  // coins = coins.filter(x => x.covenant.type == 9)
  // console.log(coins.length)
  // var tt = coins.filter(x => x.value === '7220.457271')
  // console.log(coins[coins.length-1].toJSON())
  // console.log(coins[coins.length-1].covenant.type == 3)
  // bids = bids.slice(0, 5000)
  for (i=900,j=bids.length; i<j; i+=chunk) {
    console.log(i)
    temparray = bids.slice(i,i+chunk);
    var mtx = await this.revealBids(temparray)
    if (mtx != null) {
      try { await this.fill(mtx)
      await this.finalize(mtx)
      await console.log('zzz')
      await console.log(mtx.inputs.length)
      // break;
      this.sendMTX(mtx)
      } catch (error) {
        console.log(error)
