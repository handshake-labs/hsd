...
class Wallet extends EventEmitter {
  ...
  async fill(mtx, options) {
    ...
    await console.log('coins length is ' + coins.length)
  ...
  async makeReveal(name, acct) {
    ...
    const maxBid = bids.reverse().reduce(function(prev, current) {
      return (prev.value > current.value) ? prev : current
    }) //returns object

    const mtx = new MTX();

    for (const {prevout, own} of [maxBid]) {
  ...
  //Receives bids array as parameter, returns a mtx with reveals
  async revealBids(bids) {
    const height = this.wdb.height + 1;
    const network = this.network;
    const mtx = new MTX();
...
//creates and sends reveal transactions for all possible names
async graduallySendReveals() {
  var bids = await this.getBids();
  bids = bids.filter(x => x.value != -1 && x.own);
  ...
  for (const bid of bids) {
    if (!bid.own)
      continue;
    const ns = await this.getNameState(bid.nameHash);
    if (!ns)
      continue;
    ns.maybeExpire(height, network);
    const state = ns.state(height, network);
  ...
  for (i=0, j=filtered.length; i<j; i+=chunk) {
    ...
    var mtx = await this.revealBids(temparray)
    if (mtx != null) {
      await this.fill(mtx)
      await this.finalize(mtx)
      await console.log('Sending reveal transaction with ' +mtx.inputs.length + ' inputs')
      this.sendMTX(mtx)
    }
  }
}
...
async graduallySendRedeems() {
  const height = this.wdb.height + 1;
  const network = this.network;
  const reveals = await this.txdb.getReveals();

  var filtered = [] 
  for (const redeem of reveals) {
    const nameHash = redeem.nameHash
    const prevout = redeem.prevout
    const own = redeem.own
    const {hash, index} = prevout;
    const ns = await this.getNameState(nameHash);
    if (!ns)
      continue;
  ...
  for (i=0, j=filtered.length; i<j; i+=chunk) {
    ...
    if (mtx != null) {
      ...
      await this.fill(mtx);
      await this.finalize(mtx);
      await console.log('Sending redeem transaction with ' +mtx.inputs.length + ' inputs');
      await this.sendMTX(mtx);
    }
...
//Register many names (any number) with 500 names per tx and the same resource for all of them, useful for retrieving
//back funds from the won auctions.
async graduallyRegister(list_of_names, resource) {
  const height = this.wdb.height + 1;
  const network = this.network;

  var i, j, temparray, chunk = 500;
  for (i=0, j=list_of_names.length; i<j; i+=chunk) {
    temparray = list_of_names.slice(i,i+chunk);
    const mtx = new MTX();
    ...
    for (const name of temparray) {
      ...
      if (state !== states.CLOSED)
        continue;
      const registerTx = await this._makeRegister(name, resource)
      mtx.inputs = mtx.inputs.concat(registerTx.inputs)
      mtx.outputs = mtx.outputs.concat(registerTx.outputs)
    }
    await this.fund(mtx)
    await this.finalize(mtx)
    await this.sendMTX(mtx)
...
async graduallyFinalize(list_of_names) {
  const height = this.wdb.height + 1;
  const network = this.network;
  ...
  temparray = list_of_names.slice(i,i+chunk);
    ...
    for (const name of temparray) {
      ...
      //Checks if it's possible to make a transfer
      assert(typeof name === 'string');
      if (!rules.verifyName(name))
        fail.push(name)
      if (acct != null) {
      ...
      const finalizeTx = await this.makeFinalize(name, null)
      success.push(name)
      mtx.inputs = mtx.inputs.concat(finalizeTx.inputs)
      mtx.outputs = mtx.outputs.concat(finalizeTx.outputs)
    }
    await this.fund(mtx)
    await this.finalize(mtx)
    await this.sendMTX(mtx)
...
async graduallyTransfer(list_of_names, address) {
  const height = this.wdb.height + 1;
  const network = this.network;
  ...
  temparray = list_of_names.slice(i,i+chunk);
    ...
    for (const name of temparray) {
      ...
      const transferTx = await this.makeTransfer(name, address, null)
      success.push(name)
      mtx.inputs = mtx.inputs.concat(transferTx.inputs)
      mtx.outputs = mtx.outputs.concat(transferTx.outputs)
    }
    await this.fund(mtx)
    await this.finalize(mtx)
    await this.sendMTX(mtx)
...
async registerMany(list_of_names, resource) {
  const mtx = new MTX();
  for (const name of list_of_names) {
    const registerTX = await this._makeRegister(name, resource)
    mtx.inputs = mtx.inputs.concat(registerTX.inputs)
    mtx.outputs = mtx.outputs.concat(registerTX.outputs)
  }
